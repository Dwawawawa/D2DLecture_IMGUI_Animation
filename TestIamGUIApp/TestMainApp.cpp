...
void TestMainApp::UpdateLogic()
{
    ...
    {
        // 디버깅을 위한 출력
        //int size_needed = WideCharToMultiByte(CP_UTF8, 0, &m_selectedAssetKey[0], (int)m_selectedAssetKey.size(), NULL, 0, NULL, NULL);
        //std::string strTo(size_needed, 0);
        //WideCharToMultiByte(CP_UTF8, 0, &m_selectedAssetKey[0], (int)m_selectedAssetKey.size(), &strTo[0], size_needed, NULL, NULL);
...
//// clips
//// 선택한 애니메이션만 렌더링하는 경우
void TestMainApp::RenderAssetManager_Temp()
{
    ...
    if(ext == L".png")
    {
        ID2D1Bitmap1* specificTexture = m_AssetManager.GetTexture(m_selectedAssetKey);
        if (specificTexture != nullptr)
        {
            D2D1_SIZE_F size = specificTexture->GetSize();
            D2D1_RECT_F renderRect = D2D1::RectF(100, 100, 100 + size.width, 100 + size.height);
            D2D1_RECT_F sourceRect = D2D1::RectF(0, 0, size.width, size.height);

            m_Renderer->DrawBitmap(specificTexture, renderRect, sourceRect);
        }
...
{
    std::filesystem::path fullPath = m_folderPath / std::filesystem::path(m_selectedFile);

    ext = fullPath.extension();
    if (ext.empty()) return; // 확장자가 없으면 리턴

    std::filesystem::path keyPath = fullPath.stem().wstring();
